...
class SEPALastschrift
{
     ...
     * __construct
     *
     * @param  mixed $date class DateTime
     * @param  mixed $msgid ist ein Prefix, welches auch für sie Sammler (PmtInfId) als Prefix verwendet wird. Daher die Länge auf 29 beschränken!
     * @param  mixed $initiator
     * @param  mixed $creditorName
     * @param  mixed $creditorIBAN
     * @param  mixed $creditorBIC
     * @param  mixed $creditorID GläubigerID
     * @param  mixed $ccy
     * @param  mixed $cdtype B2B or COR1
     * @param  mixed $BtchBookg BatchBookingIndicator
     * @return void
     */
    public function __construct($date, $msgid, $initiator, $creditorName, $creditorIBAN, $creditorBIC, $creditorID, $ccy="EUR", $cdtype="CORE", $BtchBookg = null)
    {
        $this->date = $date;
        $this->msgid = $msgid;
        ...
        if (!preg_match("#^([A-Za-z0-9]|[\+|\?|/|\-|:|\(|\)|\.|,|'| ]){1,29}$#", $msgid)) {
            throw new Exception("ungültige msgid");
        }
    }
    ...
    public function addLastschrift($id, $iban, $bic, $name, $mandate, $mandateSignatureDate /* class DateTime */, $amount /* float */, $subject, $type, $UltmtDbtr = null)
    {
        ...
        if (!preg_match("#^([A-Za-z0-9]|[\+|\?|/|\-|:|\(|\)|\.|,|']){1,35}$#", $mandate)) {
            throw new Exception("invalid mandate $mandate");
        }
        ...
        if (!preg_match("#^[A-Za-z0-9äöüÄÖÜß\+\?/\-:\(\)\.,' ]{1,140}$#", $subject)) {
            throw new Exception("invalid subject $subject: #^[A-Za-z0-9\+\?/\-:\(\)\.,' ]{1,140}$#");
        }
        if (strlen($name) < 1 or strlen($name) > 70) {
            throw new Exception("invalid name length");
        }
        # Sparkasse Arnstadt-Ilmenau erlaubt hier nur a-zA-Z0-9 sowie -':?,+()/. und Leerzeichen
        if (!preg_match("#^[A-Za-z0-9äöüÄÖÜß\+\?/\-:\(\)\.,' ]{1,70}$#", $name)) {
            throw new Exception("invalid name $name: #^[A-Za-z0-9\+\?/\-:\(\)\.,' ]{1,70}$#");
        }
        if ($UltmtDbtr !== null && (strlen($UltmtDbtr) < 1 or strlen($UltmtDbtr) > 70)) {
            throw new Exception("invalid UltmtDbtr length");
        }
